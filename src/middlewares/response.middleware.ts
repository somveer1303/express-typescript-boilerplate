...
export default (req: Request, res: CustomResponse, next: NextFunction) => {
  ...
  res.invalid = ({ msg, code }) =>
    res.status(200).json(sendResponse({ ok: false, err: msg || 'Invalid Parameters', code, data: null }, isEncryption));
  ...
  res.unauthorized = ({ msg }) =>
    res.status(401).json(sendResponse({ ok: false, err: msg || 'Authentication Failed', data: null }, isEncryption));

  res.success = ({ data = {} }) => res.status(200).json(sendResponse({ ok: true, err: null, data }, isEncryption));

  res.sendEncryptedData = ({ data, code = 200 }) =>
    res.status(code).send(encrypt(JSON.stringify(data), config.rsaKeys.app.publicKey, undefined));
