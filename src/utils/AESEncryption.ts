...
export function aes256cbcDecryption(
  ...
  return Buffer.concat([aes256DecryptedKey, aes256Key.final()]).toString('utf8');
}
...
export function getDecryptedValue(base64Data: string): string {
  ...
  const base64EncryptionKey1 = getBase64DecodedValue2(config.encryption.encryptionKey1);
  const base64EncryptionKey2 = getBase64DecodedValue2(config.encryption.encryptionKey2);
  const len = input.length;
  const iv = input.slice(0, 16);
  const SHA512Value = input.slice(16, 80);
  const encryptAES256CBCValue = input.slice(80, len);
  const value = aes256cbcDecryption(base64EncryptionKey1.slice(0, 32), iv, encryptAES256CBCValue);
  const newSHA512Value = generateSHA512Hash(encryptAES256CBCValue, base64EncryptionKey2);
...
export function getEncryptedValue(input: string): string {
  ...
  const SHA512Value = generateSHA512Hash(Buffer.from(encryptAES256CBCValue), base64EncryptionKey2);
  const output = Buffer.concat([uniqueId, SHA512Value, encryptAES256CBCValue]);
  return output.toString('base64');
}

export function aes256cbcEncryption(key: Buffer, initVector: Buffer, value: Buffer): Buffer {
