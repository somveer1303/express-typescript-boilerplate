...
export function encrypt(payload: any, publicKey: string | undefined, keyPath: string | undefined): string {
  if (!(publicKey || keyPath)) {
    // return Error('Please provide the private key or the path to the private key');
    return null;
  }

  let key = publicKey ? Buffer.from(publicKey, 'base64').toString('utf8') : null;
  if (!key) {
...
export function decrypt(payload: Base64, privateKey: Base64 | undefined, keyPath: string | undefined): string {
  if (!(privateKey || keyPath)) {
    // return Error('Please provide the private key or the path to the private key');
    return null;
  }

  let key = privateKey ? Buffer.from(privateKey, 'base64').toString('utf8') : null;
  if (!key) {
    const absolutePath = path.resolve(keyPath);
    key = fs.readFileSync(absolutePath, 'utf8');
  }
